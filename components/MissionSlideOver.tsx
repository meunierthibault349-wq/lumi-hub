...
export default function MissionSlideOver({ mission, onClose }: { mission: Mission; onClose: () => void }) {
    ...
    <>
      ...
      <div className="slide-over open">
        <div style={{ padding: '20px 24px 16px', borderBottom: '1px solid rgba(255,255,255,.06)', display: 'flex', alignItems: 'flex-start', gap: 12 }}>
          <div style={{ flex: 1 }}>
            <div style={{ fontFamily: 'var(--font-jakarta)', fontWeight: 700, fontSize: 16, lineHeight: 1.3 }}>{mission.title}</div>
            <div style={{ marginTop: 6 }}>
              <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '3px 10px', borderRadius: 20, background: 'var(--night-3)', border: '1px solid rgba(255,255,255,.06)', fontSize: 12, color: 'var(--gray)' }}>
                <span style={{ display: 'inline-block', width: 8, height: 8, borderRadius: '50%', background: mission.clientColor }} />
                {mission.client}
              </span>
            </div>
          </div>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '20px 24px', display: 'flex', flexDirection: 'column', gap: 20 }}>
          <div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
              ...
              <select className="form-select" style={{ width: 'auto' }} value={status} onChange={e => setStatus(e.target.value as Mission['status'])}>
          ...
          <div>
            ...
            {[['Référence', mission.ref], ['Devis', mission.devis], ['Deadline', mission.deadline]].map(([l, v]) => (
          ...
          <div>
            ...
            <div style={{ fontSize: 13, color: 'var(--gray)', lineHeight: 1.6, background: 'var(--night-3)', borderRadius: 8, padding: 12 }}>{mission.summary}</div>
          </div>
        ...
        <div style={{ padding: '16px 24px', borderTop: '1px solid rgba(255,255,255,.06)', display: 'flex', gap: 10 }}>
          ...
          <button className="btn primary" style={{ flex: 1 }}>Sauvegarder</button>
        </div>
      </div>
    </>
  );
